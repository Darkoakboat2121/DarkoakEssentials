...
import { sendData } from "./external/plugins"
import { cleanMapsAndSets } from "./data/cleaner"
...
world.afterEvents.entityHitEntity.subscribe((evd) => {
    ...
    worldSettings.hitTracking(evd)
    customCombatSystem(evd)
...
// world.beforeEvents.entityHurt.subscribe((evd) => {
//     anticheat.antiReach2(evd)
// })
...
world.afterEvents.entityDie.subscribe((evd) => {
    ...
    worldSettings.killTracking(evd)
...
system.runTimeout(() => {
    beforeEvents.asyncPlayerJoin.subscribe((evd) => {
        try {
            mcl.debugLog(`PREJOINS: ${evd?.name || 'EMPTY'}`, prejoinSystem(evd))
        } catch (e) {
...
system.runInterval(() => {
    ...
    crashJob(players)
    cleanMapsAndSets(players)
    playerDataLogger(players)
    ...
    for (let index = 0; index < players.length; index++) {
        ...
        actionBar(player)
        anticheat.anticheatMain(player, players)
...
function itemOpeners(evd) {
    ...
    if (item.typeId == 'darkoak:sitter_tool') {
...
function gens() {
    ...
    for (let index = 0; index < blocks.length; index++) {
        ...
        /**@type {string[]} */
        const possible = b?.block?.split(',')?.map(e => e?.trim()).filter(e => e)
        if (possible === undefined || possible.length < 1) continue
        const btp = possible[mcl.xorRandomNum(0, possible.length - 1)]
                        ...
                        for (let z = minZ; z <= maxZ; z++) {
                            try {
                                ...
                                if (b?.mixspread) {
                                    ...
                                    block.setBlockType({ x, y, z }, possible[mcl.xorRandomNum(0, possible.length - 1, seed)])
                                }
                            } catch {
                                mcl.adminMessage(`Failed To Set Block ${b.block} At ${x} ${y} ${z}`)
                            }
                        }
                    }
                }

                if (b?.mixspread && spread.length > 0) {
                    mcl.arraySpreader(spread, b?.delay / 20, (e, i) => {
                        if (block.getBlock({ x: e.x, y: e.y, z: e.z })) {
                            block.setBlockType({ x: e.x, y: e.y, z: e.z }, e.block)
                        }
                    })
                }
            } else {
                block.runCommand(`fill ${b.coords} ${b.coords2} ${btp}`)
            }
        }
    }
    ...
    for (let index = 0; index < mobs.length; index++) {
        ...
        // }
        try {
            if (!mcl.tickTimer(((m?.interval ?? 1) * 60) * 20)) continue

            const spawn = world.getDimension(m?.dimension ?? 'overworld')

            let summoned = spawn.getEntities({ tags: [m?.id ?? 'old'] })
            while (summoned.length < parseInt(m?.max)) {
                const loc = mcl.stringToLoc(m?.loc)
                if (m?.maxsummon ?? false) {
                    if (m?.mob) {
                        try {
                            const ent = spawn.spawnEntity(m?.mob, {
                                x: loc.x,
                                y: loc.y,
                                z: loc.z,
                            })
                    ...
                    try {
                        if (m?.mob) spawn.spawnEntity(m?.mob, {
                            x: loc.x,
                            y: loc.y,
                            z: loc.z,
                        }).addTag(m?.id ?? 'old')
            ...
            mcl.debugLog('mob spawner', String(e))
        }
    }
}

function keepMobGensNear() {
    if (!mcl.tickTimer(40)) return
    const mobs = mcl.listGetBoth('darkoak:mobgen:')

    mcl.arraySpreader(mobs, 2, (e, i) => {
        const m = JSON.parse(e.value)

        const spawn = world.getDimension(m?.dimension ?? 'overworld')
        const summoned = spawn.getEntities({ tags: [m?.id ?? 'old'] })
        if (summoned && summoned.length > 0) {
            for (let index = 0; index < summoned.length; index++) {
                const s = summoned[index]
                const center = mcl.stringToLoc(m?.loc)
                const dist = mcl.distance(center, s.location)
                const bound = (m?.bounds ?? 10)
                if (dist > bound) s.applyImpulse({
                    x: -(s.location.x - center.x) / (bound - 1),
                    y: 0.1,
                    z: -(s.location.z - center.z) / (bound - 1),
                })
            }
        }
    })
 ...
 * @param {Player} player
 */
function landclaimBorders(player) {
    if (!mcl.tickTimer(10)) return

    const community = mcl.jsonWGet('darkoak:community:general')
    ...
    for (let index = 0; index < lcs.length; index++) {
        ...
        if (l.x >= minX && l.x <= maxX && l.z >= minZ && l.z <= maxZ && l.y > -64) {
        ...
        if (community?.landclaimsborder && l.y > -64 && mcl.tickTimer(20)) {
            mcl.particleOutline({
                x: area.p1.x,
                y: l.y,
                z: area.p1.z
...
function scriptEvents(evd) {
    ...
    if (evd.id == 'darkoak:command') {
        ...
        try {
            ...
            mcl.adminMessage(`Scriptevent darkoak:command From Entity ${player.nameTag} At ${player.location.x.toFixed(0)} ${player.location.y.toFixed(0)} ${player.location.z.toFixed(0)} Has An Error: (${String(e)})`)
            return
        }
    }
...
function actionBar(player) {
    ...
    let tts = []
    if (text2 && text2?.lines && text2?.lines?.length > 0) {
        ...
        if (text2 && text2.lines.join('').length > 0) player.onScreenDisplay.setTitle({ rawtext: [{ "text": tts.join('\n') }] }, {
            fadeInDuration: 0,
            stayDuration: 20,
            fadeOutDuration: 100,
        })
...
/**@type {Map<string, {text: DebugText, hasBeenRendered: boolean}>} */
export let textMap = new Map()

/**Glide feather
 * @param {Player} player 
 * @param {Player[]} players 
 */
function onHold(player, players) {
    const item = mcl.getHands(player)
    if (!item) return
    if (item?.main?.typeId == 'darkoak:glide_feather' && !player.isOnGround && !player.isJumping && !player.isSneaking) {
        const view = player.getViewDirection()
        player.applyKnockback({ x: view.x / 2, z: view.z / 2 }, player.getVelocity().y * -1)
        return
    }
    if (item?.main?.typeId === 'darkoak:hitbox_tool' || item?.offhand?.typeId === 'darkoak:hitbox_tool') {
        const seen = players.filter(e => {
            const both = mcl.getHands(e)
            if (both?.main?.typeId === 'darkoak:hitbox_tool' || both?.offhand?.typeId === 'darkoak:hitbox_tool') return true
            return false
        })
        for (let index = 0; index < players.length; index++) {
            ...
            t.visibleTo = seen
            debugDrawer.addShape(t, p.dimension)

            const y = new DebugText({
                dimension: dimenLoc.dimension,
                x: p.location.x,
                y: p.location.y + 3.5,
                z: p.location.z
            }, p?.name)
            y.visibleTo = seen
            debugDrawer.addShape(y, p.dimension)
            system.runTimeout(() => {
                t.remove()
                y.remove()
            }, 2)
    ...
    if (item?.main?.typeId === 'darkoak:grid_tool' || item?.offhand?.typeId === 'darkoak:grid_tool') {
        const loc = player.location
        const size = 4
        const blocks = mcl.getBlocksByVolume(player.dimension.id, {
            x: loc.x - size,
            y: loc.y - size,
            z: loc.z - size,
        }, {
            ...
            y: loc.y + size,
            z: loc.z + size,
        }).filter(e => e.typeId !== 'minecraft:air')
        for (let index = 0; index < blocks.length; index++) {
...
function playerLister(player) {
    ...
    if (!players.includes(player.name)) {
        ...
        while (!mcl.jsonWSet('darkoak:playerlist', players)) {
            players.shift()
        }
    }
...
function customSlashCommands(evd) {
        ...
        for (let index = 0; index < players.length; index++) {
            ...
            switch (moneyfunctions) {
                ...
                case 'test':
                    ...
                    break
                case 'list':
                    if (evd?.sourceEntity) evd?.sourceEntity?.sendMessage(`${player.name} Has $${mcl.getScore(player)}`)
                    result = 4
                    break
            }
        }
    ...
    evd.customCommandRegistry.registerCommand({
        name: 'darkoak:transfer2',
        description: 'Alternative /darkoak:transfer',
        permissionLevel: CommandPermissionLevel.GameDirectors,
        mandatoryParameters: [
            {
                type: CustomCommandParamType.PlayerSelector,
        ...
        system.runTimeout(() => {
            const tr = spawnSimulatedPlayer({
                x: 0, y: 0, z: 0, dimension: mcl.customSlashDimen(evd)
            }, 'Darkoak_Transfer', GameMode.Creative)
            tr.commandPermissionLevel = CommandPermissionLevel.Owner
            for (let index = 0; index < player.length; index++) {
                const p = player[index]
                try {
                    //tr.runCommand(`transfer "${p.name}" "${hostname}" ${port}`)
                    tr.chat(`/transfer "${p.name}" "${hostname}" ${port}`)
                } catch (e) {
                    console.error(String(e))
                }
            }
    ...
    evd.customCommandRegistry.registerCommand({
        ...
        for (let index = 0; index < player.length; index++) {
            const p = player[index]
            mcl.jsonWSet(`darkoak:vars:${arrays.replacer(p, variable_name)}`, {
                value: arrays.replacer(p, data)
            })
        }
    })
    ...
    evd.customCommandRegistry.registerCommand({
        ...
        system.runTimeout(() => {
            switch (debugtype) {
                ...
                case 'debugfilter':
                    if (!optional) return
                    mcl.jsonWSet('darkoak:debug', {
                        filter: optional
                    })
                    break
                case 'playerraw':
                    const ptg = mcl.getPlayer(optional)
                    if (!ptg) {
                        ...
                        try {
                            player.sendMessage(JSON.stringify(mcl.playerToData(player)))
                        } catch (e) {
                ...
                case 'setmax':
                    try {
                        const fp = spawnSimulatedPlayer({
                            ...
                            y: player.location.y,
                            z: player.location.z
                        }, 'darkoak_setter', GameMode.Creative)
                ...
                case 'member(any)':
                    ...
                    break
                case 'deleteIDS':
                    const ids = world.getDynamicPropertyIds().filter(e => e.includes(optional ?? ''))
    ...
    }, (evd, nicktypes, nickname, admtarget) => {
        ...
        system.runTimeout(() => {
            switch (nicktypes) {
                case 'reset':
                    ...
                    if (mcl.isDOBAdmin(player) && admtarget) {
                        for (let index = 0; index < admtarget.length; index++) {
                            mcl.pRemove(admtarget[index], 'darkoak:nickname')
                        }
                    } else {
                ...
                case 'set':
                    /**@type {string} */
                    let nick = nickname
                    if (!nick || nick.includes(arrays.crasherSymbol) || nick.includes(arrays.crasherSymbol2) || nick.includes('§k')) return
                    player.sendMessage(`§aNickname Set To §r§f${nick || ''}`)
                    nick = nick?.substring(0, 20)?.replaceAll(arrays.crasherSymbol, '#')?.trim() ?? ''
                    if (mcl.isDOBAdmin(player) && admtarget) {
                        for (let index = 0; index < admtarget.length; index++) {
                            mcl.jsonPSet(admtarget[index], 'darkoak:nickname', {
                                nick: nick
                            })
                        }
            ...
            for (let index = 0; index < players.length; index++) {
                ...
                switch (inventype) {
                    case 'save':
                        ...
                        const equipment = mcl.getEquipmentArray(player)
                        for (let indexe = 0; indexe < equipmentSlots.length; indexe++) {
                            const si = equipmentSlots[indexe]
                            const eq = equipment[indexe]
                            invenA.container.setItem(si, eq)
                        }

                        // saving section
                        if (world.structureManager.getWorldStructureIds().includes(`darkoak:inventory_${ni}`)) world.structureManager.delete(`darkoak:inventory_${ni}`)
                        world.structureManager.createFromWorld(`darkoak:inventory_${ni}`, player.dimension, player.location, {
                            x: player.location.x,
                    ...
                    case 'load':
                        // loading section
                        try {
                            world.structureManager.place(`darkoak:inventory_${ni}`, player.dimension, player.location, {
                                includeBlocks: false,
                                includeEntities: true,
                            })
                            ...
                            const con = invenA2.container
                            let eqs = []
                            for (let indexs = 0; indexs < equipmentSlots.length; indexs++) {
                                eqs.push(con.getItem(equipmentSlots[indexs]))
                            }

                            mcl.setEquipment(player, eqs)
    ...
    evd.customCommandRegistry.registerEnum('darkoak:attributes', ['reset(none)', 'size(number.number)', 'health(number)', 'damage(number)', 'movement(number)', 'pushable(bool)', 'rideable(bool)', 'gravity(bool)', 'bandwidth(\'default\'|\'performance\'|\'quality\'|\'pvp\')', 'nametag(bool)', 'stackable(bool)', 'noinvis(bool)', 'lucky(bool)', 'leashable(bool)', 'heliophobia(bool)'])
    ...
    evd.customCommandRegistry.registerCommand({
        ...
        for (let index = 0; index < players.length; index++) {
            ...
            if (attribute === 'reset') {
                ...
                trigger(player, `darkoak:${attribute.split('(')[0]}${val}`)
            }
    ...
    evd.customCommandRegistry.registerEnum('darkoak:ptypes', ['join', 'list', 'leave', 'invite'])
    evd.customCommandRegistry.registerCommand({
        name: 'darkoak:p',
        ...
        if (!player) return {
            status: CustomCommandStatus.Failure,
            message: 'Needs To Run From An Entity'
        }

        switch (type) {
            case 'join': {
                const c = mcl.jsonPGet(player, 'darkoak:privatechat')
                if (c) {
                    player.sendMessage(`§cYou Are Already In A Private Chat§r`)
                } else {
                    mcl.jsonPSet(player, 'darkoak:privatechat', {
                        code: code
                    })
                    player.sendMessage(`§aYou Joined ${code}§r`)
                }
                break
            }
            case 'leave': {
                const c = mcl.jsonPGet(player, 'darkoak:privatechat')
                if (c) {
                    player.sendMessage(`§aYou Left ${c.code}§r`)
                    mcl.pRemove(player, 'darkoak:privatechat')
                } else {
                    player.sendMessage(`§cYou Aren't In A Private Chat§r`)
                }
                break
            }
            case 'invite': {
                const c = mcl.jsonPGet(player, 'darkoak:privatechat')
                if (!c) {
                    player.sendMessage(`§cYou Are Not In A Private Chat§r`)
                } else {
                    const invited = mcl.getPlayer(code)
                    if (!invited) return
                    invited.sendMessage(`§aYou Have Been Invited To: "${c.code}", To Join It Use /p join "${c.code}"`)
                }
                break
            }
            case 'list': {
                if (!mcl.isDOBAdmin(player)) return {
                    status: CustomCommandStatus.Failure,
                    message: 'Only Admins Can List Active Chats'
                }
                // let codes = new Set()
                /**@type {Map<string, {code: string, players: string[]}>} */
                let message = new Map()

                const players = world.getAllPlayers()
                for (let index = 0; index < players.length; index++) {
                    const p = players[index]
                    const chat = mcl.jsonPGet(p, 'darkoak:privatechat')
                    if (!chat) continue

                    if (!message.has(chat.code)) {
                        message.set(chat.code, {
                            code: chat.code,
                            players: []
                        })
                    }
                    message.get(chat.code).players.push(p.name)

                    // if (codes.has(chat.code)) {
                    //     let pls = message.get(chat.code)?.players
                    //     pls.push(p?.name)
                    //     message.set(chat.code, {
                    //         code: chat.code,
                    //         players: pls
                ...
                const actMessage = Array.from(message).map(e => `§a${e[1].code}§8: §r${e[1].players?.join(' | ')}`)?.join('\n')
                player.sendMessage(actMessage)
                break
            }
        }


        // adds to a map (code, og.push(player.name)), gets original = og
    })

    evd.customCommandRegistry.registerCommand({
        name: 'darkoak:outline',
        description: 'Makes A Particle Outline',
        permissionLevel: CommandPermissionLevel.GameDirectors,
        mandatoryParameters: [
            {
                type: CustomCommandParamType.Location,
    ...
    evd.customCommandRegistry.registerEnum('darkoak:crashtypes', ['slow', 'instant', 'chicken', 'getout'])
    evd.customCommandRegistry.registerCommand({
        name: 'darkoak:crash',
        description: 'Crashes A Player, Please Only Use This If Absolutely Necessary',
        ...
        try {
            switch (t) {
                ...
                case 'instant': {
                    system.runTimeout(() => {
                        player[0].triggerEvent('darkoak:crash')
                    })
                    break
                }
                case 'chicken': {
                    system.runTimeout(() => {
                        player[0].triggerEvent('darkoak:crash2')
                    })
                    break
                }
                case 'getout': {
            ...
            mcl.debugLog('crash command', String(e))
            r = 2
        }
    ...
    evd.customCommandRegistry.registerEnum('darkoak:bundleoptions', ['run', 'modify'])
    evd.customCommandRegistry.registerCommand({
        name: 'darkoak:bundle',
        description: 'Bundles Multiple Commands Together Into One',
        permissionLevel: CommandPermissionLevel.GameDirectors,
        mandatoryParameters: [
            {
                type: CustomCommandParamType.Enum,
                name: 'darkoak:bundleoptions'
            }
        ],
        optionalParameters: [
            {
        ...
        const toRun = evd?.initiator ?? evd?.sourceBlock ?? evd?.sourceEntity ?? mcl.customSlashDimen(evd)
        ...
        switch (type) {
            case 'run': {
                /**@type {string[]} */
                const bundled = mcl.jsonListGetBoth('darkoak:bundle:').filter(e => e?.value?.name?.startsWith(command.split(' ')[0]))[0]?.value?.commands?.filter(e => e)
                if (!bundled) {
                    r = 'nobundle'
                    break
                }
                ...
                for (let index = 0; index < bundled.length; index++) {
                    const c = bundled[index]
                    let ctr = replaceArgs(c)
                    system.runTimeout(() => {
                        if (toRun instanceof Entity) {
                            try {
                                toRun.runCommand(arrays.replacer(toRun, ctr))
                            } catch (e) {
                                console.error(String(e))
                            }
                        } else if ((toRun instanceof Block)) {
                            try {
                                toRun.dimension.runCommand(arrays.replacer(toRun, ctr))
                            } catch (e) {
                                console.error(String(e))
                            }
                        } else {
                            try {
                                toRun.runCommand(ctr)
                            } catch (e) {
                                console.error(String(e))
                            }
                        }
                ...
                function replaceArgs(com) {
                    const args = command.match(/"[^"]*"|\S+/g)?.map(arg => arg.replace(/"/g, '')) || []
                    return com.replace(/\$(\d+)\$/g, (_, index) => args[parseInt(index)] || '')
                }
                break
            }
            case 'modify': {
                if (toRun instanceof Player) {
                    system.runTimeout(() => {
                        interfacesTwo.bundleCommandMainUI(toRun)
                    })
                }
                break
            }
    ...
    evd.customCommandRegistry.registerCommand({
        ...
        let dura = main?.getComponent(ItemComponentTypes.Durability)?.damage ?? 0

        system.runTimeout(() => {
            if (main && (
                main.getLore().length > 0 ||
                main.getDynamicPropertyIds().length > 0 ||
                main.getCanDestroy().length > 0 ||
                main.getCanPlaceOn().length > 0 ||
                main.keepOnDeath ||
                main.lockMode != ItemLockMode.none ||
                main.hasComponent(ItemComponentTypes.Book) ||
                main.hasComponent(ItemComponentTypes.Dyeable) ||
                main.hasComponent(ItemComponentTypes.Enchantable) ||
                main.hasComponent(ItemComponentTypes.Inventory) ||
                main.typeId === 'minecraft:filled_map'
            )
            ) {
                player?.sendMessage('§cThe Item In Your Main Hand Cannot Be Put In Your Offhand. If The Item Can Be Put In The Offhand With The Inventory, Please Use That§r')
                return
            }

            player.runCommand(`replaceitem entity @s slot.weapon.offhand 0 ${main?.typeId ?? 'minecraft:air'} ${main?.amount ?? 1} ${dura}`)
            eq.setEquipment(EquipmentSlot.Mainhand, offhand)
            //eq.getEquipmentSlot(EquipmentSlot.Offhand).setItem(main)
            //eq.setEquipment(EquipmentSlot.Mainhand, offhand)
    ...
    // evd.customCommandRegistry.registerCommand({
    //     name: 'darkoak:reset',
    //     description: 'Resets The Addon In Case Of Emergency, Requires You To Be The Owner And Have The Reset Key',
    //     permissionLevel: CommandPermissionLevel.Any,
    //     mandatoryParameters: [
    //         {
    //             type: CustomCommandParamType.Integer,
    ...
    //     try {
    //         const resetInfo = JSON.parse(world.getDynamicProperty('darkoak:setup'))
    //         if (!resetInfo) {
    //             player.sendMessage('Sorry But There Was No Reset Data')
    //             return
    //         }
    //         if (player.name === mcl.stringEncrypt(resetInfo.hostName, true) && key === parseInt(mcl.stringEncrypt(resetInfo.resetKey, true))) {
    //             world.sendMessage('RESETTING ALL DATA')
    ...
    // evd.customCommandRegistry.registerEnum('darkoak:locateroptions', ['add', 'remove', 'removeall'])
    // evd.customCommandRegistry.registerCommand({
    //     name: 'darkoak:locaterbar',
    //     description: 'Changes Players Locater Bars',
    //     permissionLevel: CommandPermissionLevel.GameDirectors,
    //     mandatoryParameters: [
    //         {
    //             type: CustomCommandParamType.PlayerSelector,
    ...
    evd.customCommandRegistry.registerCommand({
        ...
        // const t = new DebugText({
        //     dimension: mcl.customSlashDimen(evd),
        //     x: loc1.x,
        //     y: loc1.y,
        //     z: loc1.z
        // }, 'this is a test text display')

        // t.scale = 4
        // debugDrawer.addShape(t, mcl.customSlashDimen(evd))
        // const y = new DebugBox({
        //     dimension: mcl.customSlashDimen(evd),
        //     x: loc1.x,
        //     y: loc1.y,
        //     z: loc1.z
